...
from METHODS import PowerMethodSolver2DRectFreeMF
...
class SolverFactory:
    ...
    @staticmethod
    def get_solver_power2DRectFreeMF(solver_type, group, N, conv, D, TOT, SIGS, chi, NUFIS, dx, dy, I_max, J_max, BC, precond, tol):
        if solver_type in ['forward', 'adjoint']:
            return PowerMethodSolver2DRectFreeMF(group, N, conv, D, TOT, SIGS, chi, NUFIS, dx, dy, I_max, J_max, BC, precond, tol)
        else:
            raise ValueError(f"Unknown solver type: {solver_type}")
