...
def plot_1D_centerline_y0_2(PHI1g, PHI2g, conv, I_max, J_max, dx, dy, g, varname=None, process_data=None):
    ...
    tolerance = 3  # Define a small tolerance for floating point comparisons
    ...
    ax1.annotate(f'AVS Source', 
             ...
             xytext=(peak_distance+5, peak_value+0.00005),
...
from OBJECTIVES3_TEST05_3DMG_CSTest09_CENTER_AVS import *
...
dPHI_pk_reshaped = dPHI_pk_array.reshape(group, K_max, N1)
...
dPHI_spatial_reshaped = dPHI_spatial_array.reshape(group, K_max, N1)

print(len(D[0][50]))
k = 26
D1 = [D[g][k] for g in range(len(D))]

# Make neighbor indexes
conv = convert_index_2D_rect(D1, I_max, J_max)
conv_array = np.array(conv)
##################################################################
for g in range(group):
    plot_1D_centerline_y0_2(dPHI_pk_reshaped[g][26], dPHI_spatial_reshaped[g][26], conv, I_max, J_max, dx, dy, g+1, varname='dPHI', process_data='magnitude')
    plot_1D_centerline_y0_2(dPHI_pk_reshaped[g][26], dPHI_spatial_reshaped[g][26], conv, I_max, J_max, dx, dy, g+1, varname='dPHI', process_data='phase')
