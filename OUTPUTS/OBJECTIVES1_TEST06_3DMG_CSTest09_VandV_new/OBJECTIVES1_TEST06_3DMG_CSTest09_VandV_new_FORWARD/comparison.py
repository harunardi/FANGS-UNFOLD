...
for g in range(group):
    image_files = []
    for k in range(K_max):
        filename_PHI = plot_heatmap_3D(FLX_CORESIM_new_reshaped[g, k, :, :], g+1, k+1, x, y, cmap='viridis', varname='FLX', title=f'2D Plot of FLX{g+1}, Z={k+1}', case_name=case_name, process_data='magnitude', solve='FORWARD')
...
for g in range(group):
    image_files = []
    for k in range(K_max):
        filename_PHI = plot_heatmap_3D(diff_flx_CS_reshaped[g, k, :, :], g+1, k+1, x, y, cmap='viridis', varname='diff_PHI', title=f'2D Plot of diff_PHI{g+1}, Z={k+1} in %', case_name=case_name, process_data='magnitude', solve='FORWARD')
        image_files.append(filename_PHI)
