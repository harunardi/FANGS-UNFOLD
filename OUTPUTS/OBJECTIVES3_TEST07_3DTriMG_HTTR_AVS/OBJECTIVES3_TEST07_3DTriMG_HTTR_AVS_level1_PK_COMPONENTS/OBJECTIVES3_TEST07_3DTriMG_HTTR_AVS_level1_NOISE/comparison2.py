...
def plot_1D_centerline_y0_2(PHI1g, PHI2g, x_coords, y_coords, conv_tri, I_max, J_max, g, level, varname=None, process_data=None):
    ...
    peak_index = 14
    print(peak_index, unique_distances[peak_index], flux2_values[peak_index])
    peak_distance = 24.0
    ...
    ax1.annotate(f'AVS Source', 
             ...
             xytext=(peak_distance + 5, peak_value+0.00045),
             arrowprops=dict(arrowstyle='->', color='g'),
             color='g')

    ax1.set_xlabel('Distance to Core Center (cm)')
    ax1.set_ylabel(f'{process_data.capitalize()} dPHI{g}')
    ax1.set_title(fr'{process_data.capitalize()} $\delta \phi_{{{g}}}^{{\text{{pk}}}}$ and $\delta \phi_{{{g}}}^{{\text{{spatial}}}}$ at Centerline (y=0), Midplane (Z=5)')
...
from OBJECTIVES3_TEST03_2DTriMG_HTTR2G_AVS import *
conv_hexx_2D = convert_2D_hexx(I_max, J_max, D)
conv_tri_2D, conv_hexx_ext_2D = convert_2D_tri(I_max, J_max, conv_hexx_2D, level)
conv_tri_array = np.array(conv_tri_2D)
conv_neighbor, tri_indices, x, y, all_triangles = calculate_neighbors_2D(s, I_max, J_max, conv_hexx_2D, level)
N_hexx1 = I_max * J_max * 6 * (4 ** (level - 1))

from OBJECTIVES3_TEST07_3DTriMG_HTTR_AVS import *
...
dPHI_pk_reshaped = dPHI_pk_array.reshape(group, K_max, N_hexx1)
...
dPHI_spatial_reshaped = dPHI_spatial_array.reshape(group, K_max, N_hexx1)

##################################################################
for g in range(group):
    plot_1D_centerline_y0_2(dPHI_pk_reshaped[g][4], dPHI_spatial_reshaped[g][4], x, y, conv_tri_2D, I_max, J_max, g+1, level, varname='dPHI', process_data='magnitude')
    plot_1D_centerline_y0_2(dPHI_pk_reshaped[g][4], dPHI_spatial_reshaped[g][4], x, y, conv_tri_2D, I_max, J_max, g+1, level, varname='dPHI', process_data='phase')
