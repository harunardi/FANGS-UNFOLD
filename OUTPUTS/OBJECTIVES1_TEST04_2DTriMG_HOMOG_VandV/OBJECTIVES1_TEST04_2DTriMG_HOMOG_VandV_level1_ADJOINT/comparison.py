...
#    plt.ylabel('Adjoint Flux Values (normalized)')
#    plt.title('Adjoint Flux Values vs. Distance to Core Center')
#
...
#    plt.savefig(f'Verification_{varname}_{process_data}_G{g}.png')
    ...
    ax1.set_ylabel(f'Adjoint Flux Group {g} Values (normalized)')
    ax1.set_title(f'Adjoint Flux Group {g} Values vs. Distance to Core Center')
...
keff_ADJ = adjoint_output["keff"]
PHI1_ADJ = adjoint_output["PHI1_ADJOINT"]
PHI2_ADJ = adjoint_output["PHI2_ADJOINT"]
PHI_ADJ = PHI1_ADJ + PHI2_ADJ
PHI_ADJ_array = np.array(PHI_ADJ)
PHI_ADJ_reshaped = PHI_ADJ_array.reshape(group, N_hexx)
...
c_mu = (1/keff) * nuSigma_f2/(Sigma_a2 + (D2 * B_g**2))

J0_values = j0(B_g * r)
FLX1_ADJ = 1 * J0_values
FLX2_ADJ = c_mu * J0_values

FLX_ADJ = [FLX1_ADJ, FLX2_ADJ]/(max(FLX2_ADJ))

#################################################################
for g in range(group):
    plot_1D_distance_to_core(PHI_ADJ_reshaped[g], FLX_ADJ[g], h, I_max, J_max, g+1, level, varname='PHI_ADJ', process_data='magnitude')

#for g in range(group):
#    if level == 1:
#        plot_triangular_level1(PHI_ADJ_reshaped[g], h, I_max, J_max, g+1, level, cmap='viridis', varname='PHI_ADJ', title=f'2D Plot of PHI{g+1}_ADJ Hexx', case_name=case_name, output_dir=output_dir, solve="adjoint", process_data="magnitude")
#        plot_1D_distance_to_core(PHI_ADJ_reshaped[g], h, I_max, J_max, g+1, level, varname='PHI_ADJ', case_name=case_name, output_dir=output_dir, solve="adjoint")
#    elif level == 2:
#        plot_triangular_level2(PHI_ADJ_reshaped[g], h, I_max, J_max, g+1, level, cmap='viridis', varname='PHI_ADJ', title=f'2D Plot of PHI{g+1}_ADJ Hexx', case_name=case_name, output_dir=output_dir, solve="adjoint", process_data="magnitude")
#    elif level == 3:
#        plot_triangular_level3(PHI_ADJ_reshaped[g], h, I_max, J_max, g+1, level, cmap='viridis', varname='PHI_ADJ', title=f'2D Plot of PHI{g+1}_ADJ Hexx', case_name=case_name, output_dir=output_dir, solve="adjoint", process_data="magnitude")
#    elif level == 4:
#        plot_triangular_level4(PHI_ADJ_reshaped[g], h, I_max, J_max, g+1, level, cmap='viridis', varname='PHI_ADJ', title=f'2D Plot of PHI{g+1}_ADJ Hexx', case_name=case_name, output_dir=output_dir, solve="adjoint", process_data="magnitude")
#